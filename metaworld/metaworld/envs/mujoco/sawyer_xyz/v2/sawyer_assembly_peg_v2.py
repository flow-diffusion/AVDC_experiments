...
class SawyerNutAssemblyEnvV2(SawyerXYZEnv):
    ...
    def __init__(self):
        hand_randomness = np.concatenate([np.random.uniform(-0.3, 0.3, size=2), np.random.uniform(0, 0.2, size=1)])
        obj_randomness = np.concatenate([np.random.uniform(-0.1, 0.1, size=2), [0]])
        ...
        self.init_config = {
            'obj_init_angle': 0.3,
            'obj_init_pos': np.array([0, 0.6, 0.02], dtype=np.float32) + obj_randomness,
            'hand_init_pos': np.array((0, 0.6, 0.2), dtype=np.float32) + hand_randomness,
        }
